...
This page is part of the complete Jeston documentation. It explains build output, environment, process managers, containers, proxies, health checks, and rollback. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

## Portable deployment

Build once and run the generated artifact with the supported Node version. Provide environment variables through the platform secret manager. Put a reverse proxy or load balancer in front of the process when TLS termination, compression, request buffering, or multi-instance routing is required.

The deployment must expose a readiness check, a liveness strategy, structured logs, and a rollback procedure. If sessions or cache are process-local, configure sticky routing or move those concerns to a shared provider before adding instances.
...
<Info>
**This page is a working surface, not a glossary.** It explains build artifacts, environment injection, process topology, probes, and rollback. Use the examples as a review checklist: every boundary should have an owner, a failure mode, a measurable signal, and a rollback or recovery story.
</Info>

<Columns cols={2}>
  <Card title="Primary outcome" icon="cloud-arrow-up" type="check">
