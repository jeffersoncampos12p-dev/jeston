...
This page is part of the complete Jeston documentation. It explains methodology, latency percentiles, concurrency, memory, cold starts, databases, reproducibility, and interpretation. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

## Methodology

A useful benchmark declares the hardware, Node version, build mode, route mix, payload sizes, dependency topology, concurrency model, warm-up policy, duration, and success criteria. Report requests per second together with p50, p95, p99, error rate, CPU, RSS, heap, database connections, and queue depth.

A single synthetic endpoint can reveal runtime overhead, but it cannot prove application performance. Include representative SSR, API, streaming, cache miss, cache hit, SQL, authentication, and external-network workloads. Repeat the test, publish the raw parameters, and avoid comparing results produced by different topologies.
...
<Info>
**This page is a working surface, not a glossary.** It explains workload design, percentiles, concurrency, saturation, memory, and honest comparisons. Use the examples as a review checklist: every boundary should have an owner, a failure mode, a measurable signal, and a rollback or recovery story.
</Info>

<Columns cols={2}>
  <Card title="Primary outcome" icon="gauge-high" type="check">
