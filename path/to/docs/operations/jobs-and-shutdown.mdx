...
This page is part of the complete Jeston documentation. It explains queues, retries, idempotency, signals, draining, deadlines, dead letters, and deploy coordination. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
<Info>
**This page is a working surface, not a glossary.** It explains queues, retries, idempotency, signals, draining, and bounded shutdown. Use the examples as a review checklist: every boundary should have an owner, a failure mode, a measurable signal, and a rollback or recovery story.
</Info>

<Columns cols={2}>
  <Card title="Primary outcome" icon="power-off" type="check">
