...
| --- | --- |
| **Focus** | Turn a source change into a release only after code, artifacts, compatibility, and provenance agree. |
| **Deliverable** | A release gate that protects users from broken types, missing files, and unsupported runtime behavior. |
| **Reason to care** | Publishing is the last step of a chain; it should not be the first place a package is tested as a consumer would use it. |
