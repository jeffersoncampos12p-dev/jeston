...
| --- | --- | --- | --- |
| <Badge color="red" stroke />v2.0.0 | Published during the earlier Hedron package/ownership phase. The project subsequently moved to the Kvant namespace and continued development under `@kvantjs/jeston`; the 2.x runtime also introduced new routing, client, cache, security, and SEO contracts. | v2.1.x | Yes: install `@kvantjs/jeston`, review the 2.x migration notes, and rebuild generated output. |
| v1.2.0 | Superseded by the 2.x architecture. Its pre-App-Router conventions do not describe the current `app/` routing, boundary, Server Action, or client-runtime contracts. | v2.1.x | Yes for projects adopting current features; keep a pinned v1 install only for maintenance. |
| v1.1.0 | Older pre-2.x API and build contracts; no longer the compatibility target of the current source tree or release checks. | v2.1.x | Yes for active development. |
| v1.0.0 | Initial stable 1.x contract, replaced as the framework moved to the current TypeScript/ESM and full-stack runtime model. | v2.1.x | Yes for active development. |
| v0.4.0 | Pre-1.0 development release. It predates the current compatibility guarantees and should not be used as a deployment baseline. | v2.1.x | Yes. |
| v0.3.0 | Pre-1.0 development release with an obsolete API surface and no current support promise. | v2.1.x | Yes. |
| v0.2.0 | Pre-1.0 development release; its experimental contracts were replaced by the 1.x and later 2.x lines. | v2.1.x | Yes. |
| v0.1.0 | First experimental release. It is retained only to explain the project's history and is not a supported runtime. | v2.1.x | Yes. |
