...
This page is part of the complete Jeston documentation. It explains framework.config.ts, environment variables, limits, cache, security headers, logging, and adapters. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Configuration layering

Keep non-secret behavior in `framework.config.ts` and secret values in the environment. Configuration should be loaded deterministically for the current mode. Avoid reading secrets during client bundling, and never expose server-only variables through page props or generated manifests.

Configuration for limits is part of security: body bytes, request timeouts, cache entries, stream lifetime, and concurrent jobs should have finite defaults. An explicit limit makes overload observable instead of allowing unbounded memory growth.
...
> **Reading lens** — Use this page to reason about environment separation, typed options, secure defaults, and operational overrides. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="sliders" type="check">
