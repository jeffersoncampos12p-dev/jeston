...
This page is part of the complete Jeston documentation. It explains Flight protocol, client references, server actions, bundler manifests, and safe adoption. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
> **Reading lens** — Use this page to reason about server and client boundaries, serialization, cache ownership, and the current maturity line. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="atom" type="check">
