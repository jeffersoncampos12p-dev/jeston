...
<Info>
**This page is a working surface, not a glossary.** It explains server and client boundaries, serialization, cache ownership, and the current maturity line. Use the examples as a review checklist: every boundary should have an owner, a failure mode, a measurable signal, and a rollback or recovery story.
</Info>

<Columns cols={2}>
  <Card title="Primary outcome" icon="atom" type="check">
