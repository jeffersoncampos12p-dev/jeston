...
This page is part of the complete Jeston documentation. It explains RequestContext, JSON, body limits, redirects, errors, CORS, streaming, and abort signals. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Request context

An API handler receives a typed request context containing the HTTP method, URL, headers, parsed body when appropriate, request ID, deadline information, and an abort signal. The signal is the most important operational field: pass it to database drivers, fetch calls, streams, and model clients.

```ts
export const GET: ApiHandler = async ({ url, headers, signal }) => {
  const result = await fetch(url, { headers, signal });
  return { status: result.ok ? 200 : 502, json: await result.json() };
};
```

Use explicit method exports. Jeston can provide automatic `OPTIONS` and safe `HEAD` behavior, but business routes should still document which methods are supported and what authorization each method requires.
...
> **Reading lens** — Use this page to reason about method handling, validation, response semantics, cancellation, errors, and idempotency. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="brackets-curly" type="check">
