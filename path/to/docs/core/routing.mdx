...
| --- | --- |
| **Focus** | Treat route selection as a deterministic contract, especially when dynamic and catch-all paths coexist. |
| **Deliverable** | Predictable precedence, parameters, method boundaries, and 404 behavior. |
| **Reason to care** | Routing bugs are often silent: a request reaches a valid handler that was not the intended handler. |
