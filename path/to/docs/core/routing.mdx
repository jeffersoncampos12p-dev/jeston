...
This page is part of the complete Jeston documentation. It explains route discovery, parameters, precedence, pages, API methods, assets, and testing. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Route discovery and precedence

Jeston derives routes from the pages directory. Static segments are the most predictable. Dynamic segments such as `[id]` capture one decoded value. Catch-all segments such as `[...slug]` capture a sequence. Keep overlapping patterns intentional and test them with encoded values, empty values, and reserved characters.

| File | Example URL | Typical use |
| --- | --- | --- |
| `pages/index.tsx` | `/` | Landing page |
| `pages/users/[id].tsx` | `/users/42` | Resource detail |
| `pages/docs/[...slug].tsx` | `/docs/core/routing` | Documentation hierarchy |
| `pages/api/health.ts` | `/api/health` | API endpoint |

The compiler should generate a deterministic manifest. Review that manifest in CI when a route is added, renamed, or removed. Treat URL changes as compatibility changes because bookmarks, crawlers, clients, and caches may depend on them.
...
> **Reading lens** — Use this page to reason about static routes, dynamic segments, catch-all boundaries, precedence, and route-level contracts. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="route" type="check">
  ...
  <Card title="Why it matters" icon="circle-question" type="note">
    Understand the request path before adding platform complexity. Keep policy in the application and keep provider mechanics behind adapters.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="Architecture" icon="diagram-project" href="/core/architecture" horizontal>Open the guide and continue from here.</Card>
  <Card title="Routing" icon="route" href="/core/routing" horizontal>Open the guide and continue from here.</Card>
  <Card title="API routes" icon="brackets-curly" href="/core/api-routes" horizontal>Open the guide and continue from here.</Card>
</Columns>
