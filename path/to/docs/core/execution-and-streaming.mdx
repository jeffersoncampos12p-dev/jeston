...
This page is part of the complete Jeston documentation. It explains async iterables, SSE, NDJSON, React streams, timeouts, disconnects, and resource cleanup. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Streaming protocols

Choose a protocol that matches the consumer. SSE is convenient for browser event streams. NDJSON is useful for line-oriented machine consumers. A raw byte stream is appropriate when the consumer owns framing. Every stream should define content type, event schema, heartbeat policy, maximum duration, cancellation, and error behavior.

Backpressure matters. Do not generate unlimited data faster than the socket can write. Stop the iterator when the request aborts or the response closes, and release timers, database cursors, files, and provider handles in `finally` blocks.
...
> **Reading lens** — Use this page to reason about deadlines, AbortSignal, backpressure, SSE, progressive work, and graceful cancellation. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="stream" type="check">
