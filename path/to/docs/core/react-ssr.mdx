...
| --- | --- |
| **Focus** | Use server rendering to deliver useful HTML without turning hydration into an invisible second application. |
| **Deliverable** | A render path with explicit data ownership, hydration assumptions, and recovery behavior. |
| **Reason to care** | SSR performance is determined by data dependencies, serialization, cache behavior, and client takeover—not only render speed. |
