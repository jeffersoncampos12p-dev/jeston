...
This page is part of the complete Jeston documentation. It explains registry lifecycle, catalog metadata, runtime support, peer dependencies, certification, and marketplace design. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Integration contract

An integration is an independently versioned module that declares an ID, name, semantic version, category, description, supported runtimes, optional peer dependencies, and lifecycle hooks. The registry rejects duplicate IDs and malformed versions so configuration errors fail early.
...
registry.register({
  ...
  description: 'Exports structured metrics',
  runtimes: ['node'],
  setup: async ({ env }) => connect(env.ACME_METRICS_URL),
  teardown: async () => flushAndClose()
});
```

The catalog is metadata, not a promise that every provider is bundled or officially maintained. A production adapter needs contract tests, security notes, compatibility matrices, support ownership, and clear behavior during provider outages.
...
> **Reading lens** — Use this page to reason about registry metadata, provider catalogs, plugin lifecycle, capability discovery, and governance. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="plug" type="check">
  ...
  <Card title="Why it matters" icon="circle-question" type="note">
    Use stable contracts and extension points as the source of implementation truth. Keep policy in the application and keep provider mechanics behind adapters.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="Types" icon="code" href="/reference/types" horizontal>Open the guide and continue from here.</Card>
  <Card title="HTTP contracts" icon="globe" href="/reference/http-contracts" horizontal>Open the guide and continue from here.</Card>
  <Card title="Adapters" icon="puzzle-piece" href="/reference/adapters" horizontal>Open the guide and continue from here.</Card>
</Columns>
