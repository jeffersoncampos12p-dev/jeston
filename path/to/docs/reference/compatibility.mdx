...
This page is part of the complete Jeston documentation. It explains semantic versioning, supported matrices, deprecations, migrations, lockfiles, and compatibility tests. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
> **Reading lens** — Use this page to reason about Node versions, browser assumptions, React behavior, package boundaries, and migration risk. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="scale-balanced" type="check">
