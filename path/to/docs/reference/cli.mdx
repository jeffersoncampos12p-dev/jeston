...
This page is part of the complete Jeston documentation. It explains commands, flags, output directories, environment loading, exit codes, and automation. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Command workflow

Use `jeston create` to create a project, `jeston dev` for local development, `jeston build` for a deployable artifact, `jeston start` to run that artifact, and `jeston doctor` to inspect project assumptions. Use `--out-dir` when concurrent builds or isolated CI workspaces are required.

Treat CLI exit codes as an automation contract. A non-zero exit means the command did not produce a trustworthy result. CI should run typecheck, tests, build, package inspection, and the doctor command before publishing.
...
> **Reading lens** — Use this page to reason about project commands, checks, local operations, release discipline, and CI parity. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="terminal" type="check">
