...
| --- | --- |
| **Focus** | Create provider seams that preserve framework lifecycle while isolating vendor protocol and credentials. |
| **Deliverable** | Adapters with narrow capabilities, explicit cleanup, and replaceable test doubles. |
| **Reason to care** | An adapter is valuable when it makes provider choice reversible without weakening the application contract. |
