...
This page is part of the complete Jeston documentation. It explains file-based routing, React pages, API handlers, SSR, assets, and local validation. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
> **Reading lens** — Use this page to reason about a request moving from route declaration through validation, rendering, and operational feedback. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="wand-magic-sparkles" type="check">
  ...
  <Card title="Why it matters" icon="circle-question" type="note">
    Move from an empty directory to a verified, maintainable application. Keep policy in the application and keep provider mechanics behind adapters.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="Installation" icon="download" href="/start/installation" horizontal>Open the guide and continue from here.</Card>
  <Card title="First app" icon="rocket" href="/start/first-app" horizontal>Open the guide and continue from here.</Card>
  <Card title="SaaS starter" icon="building" href="/start/saas-starter" horizontal>Open the guide and continue from here.</Card>
</Columns>
