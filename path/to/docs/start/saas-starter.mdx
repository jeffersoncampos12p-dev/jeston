...
This page is part of the complete Jeston documentation. It explains tenancy, authentication, SQL, caching, jobs, observability, deployment, and failure isolation. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
> **Reading lens** — Use this page to reason about tenancy, identity, billing boundaries, background work, and the minimum operational surface. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="building" type="check">
