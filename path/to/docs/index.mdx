...
This page is part of the complete Jeston documentation. It explains orientation, architecture, installation, API design, operations, integrations, AI agents, and release engineering. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.
...
> **Reading lens** — Use this page to reason about the framework surface, the runtime contract, and the path from a first route to a production platform. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="rocket" type="check">
  ...
  <Card title="Why it matters" icon="circle-question" type="note">
    Use this map to choose the right depth for the problem in front of you. Keep policy in the application and keep provider mechanics behind adapters.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="Start here" icon="rocket" href="/start/installation" horizontal>Open the guide and continue from here.</Card>
  ...
  <Card title="Production" icon="server-stack" href="/operations/production" horizontal>Open the guide and continue from here.</Card>
</Columns>
