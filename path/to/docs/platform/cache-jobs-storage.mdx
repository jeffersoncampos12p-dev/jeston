...
This page is part of the complete Jeston documentation. It explains TTL, stale-while-revalidate, queues, idempotency, retries, object storage, and shutdown. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Cache semantics

A cache is a consistency decision. Document whether stale values are acceptable, how tags are invalidated, whether concurrent misses are deduplicated, and what happens when the backend is unavailable. Process-local caches are fast but are not shared between workers.

Jobs require a different guarantee. Persist the intent, use an idempotency key, record attempts, and make retries safe. Object storage should use bounded uploads, content validation, private-by-default objects, and lifecycle policies for temporary data.
...
> **Reading lens** — Use this page to reason about freshness, durable work, object lifecycle, bounded resources, and provider portability. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="layer-group" type="check">
