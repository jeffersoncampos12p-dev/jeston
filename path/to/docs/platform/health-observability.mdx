...
This page is part of the complete Jeston documentation. It explains health, readiness, logs, metrics, traces, request IDs, alerts, dashboards, and SLOs. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Health versus readiness

Liveness answers whether the process should be restarted. Readiness answers whether the instance should receive traffic. A database outage may make readiness fail while liveness remains healthy. Include check names, status, latency, timestamps, and safe diagnostic metadata.

Observability should connect logs, metrics, and traces through request and operation IDs. Avoid logging tokens, passwords, full prompts, or raw personal data. Define alerts from user-impacting symptoms such as error rate, p95 latency, queue age, and readiness failures.
...
> **Reading lens** — Use this page to reason about health semantics, readiness, metrics, logs, traces, and useful alert signals. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="chart-line" type="check">
