...
This page is part of the complete Jeston documentation. It explains parameterized queries, PostgreSQL, SQLite, migrations, pooling, transactions, retries, and observability. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Query discipline

Use parameterized SQL for every value. Identifiers such as table and column names cannot usually be parameters, so allow-list them before interpolation. Keep transactions short, record query duration, and pass the request signal when the driver supports cancellation.
...
Connection pools are process-local. A deployment with many processes must size the total connection count against the database limit, not against the limit of one process. Use migrations that are backward compatible with the previous application version when rolling deployments are possible.
...
> **Reading lens** — Use this page to reason about query boundaries, transactions, connection pools, migrations, and failure-aware persistence. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="database" type="check">
