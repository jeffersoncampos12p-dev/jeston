...
| --- | --- |
| **Focus** | Build agent execution as a controlled workflow with tools, budgets, traces, and evaluation—not as an unbounded chat loop. |
| **Deliverable** | Model adapters, permissioned tools, durable state, streaming, and measurable quality. |
| **Reason to care** | Agent workloads combine nondeterministic model output with deterministic application side effects. |
