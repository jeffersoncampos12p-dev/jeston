...
This page is part of the complete Jeston documentation. It explains streaming, tools, model adapters, memory, vector data, evaluations, training orchestration, and governance. The examples target `@hedronjs/jeston@1.2.0` and Node.js 20 or newer. The page distinguishes stable behavior from roadmap work so that readers can make safe production decisions.

<Accordion title="Background and detailed explanation" icon="book-open">

## Agent execution model

An agent request is a workflow with model calls, tools, memory, persistence, and policy checks. Keep each tool invocation behind an authorization boundary. Attach a trace ID to the user request, model call, tool call, queue job, and final result. Persist durable checkpoints when a run can exceed the request deadline.

```ts
export const POST: ApiHandler = async ({ body, signal, requestId }) => {
  const run = await agentRuns.start({ input: body, requestId });
  return { stream: streamAgent(run.id, { signal }) };
};
```

Do not treat model output as trusted input. Validate tool arguments, bound token and wall-clock budgets, redact secrets from prompts and traces, and define retention for prompts, completions, embeddings, and evaluation data.
...
> **Reading lens** — Use this page to reason about model adapters, tool permissions, traces, memory, evaluation, streaming, and training workloads. The goal is not to memorize APIs. The goal is to make a boundary explicit enough to implement, test, operate, and replace.

<Columns cols={2}>
  <Card title="What you will decide" icon="brain" type="check">
  ...
  <Card title="Why it matters" icon="circle-question" type="note">
    Add persistence, identity, caching, observability, and AI with explicit boundaries. Keep policy in the application and keep provider mechanics behind adapters.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="SQL" icon="database" href="/platform/sql" horizontal>Open the guide and continue from here.</Card>
  <Card title="Authentication" icon="user-lock" href="/platform/auth" horizontal>Open the guide and continue from here.</Card>
  <Card title="AI agents" icon="brain" href="/platform/ai-agents" horizontal>Open the guide and continue from here.</Card>
</Columns>
