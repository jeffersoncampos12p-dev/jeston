# Benchmark: Ryvax 0.2.0 vs. Next.js 16.3.4

## Conclusion

Ryvax 0.2.0 reduced hot-path server overhead without removing SSR, SSG, API routes, middleware, HTTP caching, security headers, or configurable observability. In the same minimal application used in the previous benchmark, optimized Ryvax reached **5,250 RPS for SSR HTML** versus **801 RPS for the Next.js Pages Router**, and **5,225 RPS for JSON API traffic** versus **2,078 RPS for Next.js**.
...
The test ran on Linux amd64 with Node.js `v22.13.0`, optimized Ryvax, and Next.js `16.3.4` using the Pages Router. Each application exposed an on-demand `/` page and an `/api/health` route. Both servers ran in production mode over loopback with 25 concurrent workers, 100 warmup requests, and a 10-second window per target.

The main Ryvax measurement disabled request IDs and request logging to avoid comparing Ryvax console I/O against a Next.js server that did not log every request.

## Main 10-second run

| Route | Framework | RPS | P50 | P95 | Maximum | Errors |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `/` | Ryvax 0.2.0 | **5,250.28** | **3.912 ms** | **8.932 ms** | 29.094 ms | 0 |
| `/` | Next.js Pages Router | 801.17 | 29.399 ms | 45.967 ms | 220.906 ms | 0 |
| `/api/health` | Ryvax 0.2.0 | **5,224.60** | **4.028 ms** | **8.530 ms** | 26.363 ms | 0 |
| `/api/health` | Next.js Pages Router | 2,078.33 | 10.866 ms | 20.701 ms | 48.529 ms | 0 |

Compared with the previous Ryvax result, SSR throughput increased by approximately **189%** and JSON throughput by approximately **200%**.

## Stability repetitions

Three additional three-second runs used the same concurrency. Ryvax reported no errors in any run.

| Run | Ryvax SSR | Next.js SSR | Ryvax API | Next.js API |
| --- | ---: | ---: | ---: | ---: |
...
The average was approximately **4,169 RPS for Ryvax SSR** versus **943 RPS for Next.js**, and **4,222 RPS for the Ryvax API** versus **2,178 RPS for Next.js**.

## Limits of the conclusion

This measurement did not include React Client Components, Server Components, streaming, hydration, databases, authentication, uploads, image optimization, CDNs, TLS, reverse proxies, or distributed caching. Next.js offers a broader integrated React surface; Ryvax offers a smaller and more explicit surface.

The defensible conclusion is: **Ryvax 0.2.0 substantially reduced its runtime overhead and outperformed the minimal Next.js Pages Router application in this local SSR and API scenario while retaining framework features.** A production decision should still use the same complete application in both frameworks.
